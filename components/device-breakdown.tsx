...
export function DeviceBreakdown({ devices }: DeviceBreakdownProps) {
  ...
  const pieChartData = [
    { name: "Allowed", value: devices.reduce((sum, device) => sum + (device.query_count - device.blocked_count), 0) },
    { name: "Blocked", value: devices.reduce((sum, device) => sum + device.blocked_count, 0) }
  ]
